...
// Reduced the robot speed to 10 times slower to capture good quality map, it takes a lot time.
// However, results are really good
#define robot_speed 0.10
...
bool robot_move(const ROBOT_MOVEMENT move_type)
{
    if (move_type == STOP) {
        ...
        motor_command.angular.z = 0.0;
        motor_command.linear.x = 0.50*robot_speed; //was 0.50
    }

    else if (move_type == BACKWARD) {
        ROS_INFO("[ROBOT] I'm going back! \n");
        motor_command.linear.x = -0.75*robot_speed; //was 0.75
        motor_command.angular.z = 0.0;
    }
        ...
        motor_command.linear.x = 0.0;
        motor_command.angular.z = 1.0*robot_speed; //was 1.0
    }
        ...
        motor_command.linear.x = 0.25*robot_speed; //was 0.25
        motor_command.angular.z = -0.25*robot_speed; //was 0.25
    }
